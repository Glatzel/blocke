    ...
    R: AsyncIRaxReader,
{
    ...
    /// sentence.
    async fn preprocess(&mut self) -> Option<(Talker, Identifier, String)> {
        loop {
    ...
    async fn dispatch_by_lines(&mut self) -> Option<(Talker, Identifier, String)> {
        loop {
            if let Some((talker, identifier, sentence)) = self.preprocess().await {
                match identifier {
...
impl<'a, R> Stream for AsyncDispatcher<'a, R>
where
    R: AsyncIRaxReader + Unpin + 'a, // `Unpin` is required for safe projection
{
    type Item = (Talker, Identifier, String);

    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        // Use `Box::pin` to turn `dispatch_by_lines()` into a future we can poll.
        let fut = self.dispatch_by_lines();
        futures::pin_mut!(fut);
        fut.poll(cx)
    }
}
