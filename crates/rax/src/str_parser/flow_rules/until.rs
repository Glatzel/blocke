    ...
    O: FromStr,
{
    ...
    fn apply(&self, _: &StrParserContext, input: &'a str) -> Option<(O, &'a str)> {
...
mod tests {
    ...
    fn test_until_str_output() {
        ...
        let result: Option<(String, &str)> = rule.apply(&ctx, ctx.full);
    ...
    fn test_until_int_output() {
        ...
        let input = "123,rest";
        let ctx = StrParserContext::new(input);
        let result: Option<(i32, &str)> = rule.apply(&ctx, ctx.full);
    ...
    fn test_until_parse_fail() {
        ...
        let input = "abc,rest";
        let ctx = StrParserContext::new(input);
        let result: Option<(bool, &str)> = rule.apply(&ctx, ctx.full);
